                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($rows as $row)
                            <tr class="hover:bg-slate-50/60 transition-colors align-top">
                                <td class="px-4 py-3 font-medium text-slate-900 whitespace-nowrap">{{ $row->name }}</td>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div @click.stop
                ...
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Edit Role' : 'New Role' }}</h2>
                ...
                <div class="mt-5 space-y-4">
                    <div>
                        <input wire:model="name" class="w-full rounded-xl border px-4 py-2" placeholder="Name">
                        @error('name')
                    ...
                    <div>
                        <input wire:model="label" class="w-full rounded-xl border px-4 py-2" placeholder="Label">
                        @error('label')
                            <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <textarea wire:model="description" rows="3" class="w-full rounded-xl border px-4 py-2" placeholder="Description"></textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="font-medium text-slate-900">Permissions</div>
    ...
    <template x-teleport="body">
        <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div @click.stop
                ...
                <p class="mt-2 text-sm text-slate-600">This action cannot be undone. Are you sure you want to delete this role?</p>
